...
function fangdajing(opt) {
    ...
    samllimg.style.height = img.length * defaultopt.Width + 'px';
    //找到遮罩的节点
    ...
    //找到lefts添加点击事件
    var lefts = box.getElementsByClassName('tops')[0];
    //找到top节点添加点击事件
    var rights = box.getElementsByClassName('bottoms')[0];
    //iW = 小图img的宽度 + 边框和内边距的和 X 移动图片的个数
    var iW = (samllimg.children[0].offsetHeight + defaultopt.BoPa) * defaultopt.speed;
    ...
    function moveimg(speed) {
        ...
        var left = samllimg.offsetTop + speed;
        //判断临界值
        if (left <= box3.offsetHeight - samllimg.offsetHeight) {
            //left = box3的宽度 - ul的宽度
            left = box3.offsetHeight - samllimg.offsetHeight;
        } else if (left >= 0) {
            left = 0;
        }
        samllimg.style.bottom = left + 'px';
    }
}
